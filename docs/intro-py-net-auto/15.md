...
有了 SNMP 和 Python，我们可以做很多事情，但那将是另外一整本书。在第 16 章中，你将探索更多的网络自动化场景。此外，您还将学习使用用于 Python 测试的虚拟环境`virtualenv`，以及 Ansible、pyATS 和 Docker、sendmail 和 Twilio SMS Python 网络自动化应用开发。