...
由于从数据库提取数据的方法，该工具不使用或存储 SQL 来执行数据提取。从用户/管理员的角度来看，这意味着完整或部分备份不应被视为“数据库转储”，因为它不包含可用于重新创建表或索引的数据。备份的主要用途是在灾难恢复或磁盘故障时作为`restore`命令的来源，或者将数据移动到新的数据库。数据库管理员应该负责提供满足数据、应用程序和业务需求的数据库备份策略。
