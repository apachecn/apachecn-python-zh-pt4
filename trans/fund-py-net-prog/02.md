...
![Image](images/sq.jpg) **注意**在 POSIX 系统上，标识套接字的`fileno()`整数也是从表示打开文件的整数池中提取的文件描述符。假设在 POSIX 环境中，您可能会遇到这样的代码，它获取这个整数，然后使用它对文件描述符执行非网络调用，如`os.read()`和`os.write()`，对实际上是网络通信端点的对象执行类似文件的操作。但是，因为本书中的代码也是为在 Windows 上工作而设计的，所以您将只对您的套接字执行真正的套接字操作。
