...
![Image](images/sq.jpg) **注**因为嘈杂的电传打字机是计算机终端的最早例子，所以 Unix 常用 TTY 作为终端设备的缩写。这就是为什么测试你的输入是否是终端的调用被命名为`isatty()`。
