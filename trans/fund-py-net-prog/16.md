...
这本书的第一部分谈了很多关于多路复用的内容——关于 UDP ( [第二章](02.html))和 TCP ( [第三章](03.html))如何采用底层 IP 协议，该协议没有考虑到实际上可能有几个用户或应用在一台计算机上需要通信，并添加了 UDP 和 TCP 端口号的概念，以便一对 IP 地址之间可以同时进行几个不同的对话。
